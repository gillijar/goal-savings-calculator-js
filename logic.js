...
clearStorageBtn.addEventListener("click", () => {
  if (storageItem && storageAmount) {
    if (
      confirm("Clicking OK will delete all current data and create a new file")
    ) {
...
backgroundImages.forEach((image) => {
  image.addEventListener("click", (e) => {
    ...
    const newBkg =
      (body.style.backgroundImage = `url(img/${selectedImage.id}.jpeg)`);
    selectedImage.classList.add("current");

    localStorage.setItem("Background Image", newBkg);
    localStorage.setItem("Current", selectedImage.id);
  });
});
